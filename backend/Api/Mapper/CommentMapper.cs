    ...
    {   
        ...
        public static CommentDTOResponse ToCommentDTOResponse(this Comment comment)
        {
            return new CommentDTOResponse
            {
                ...
                CreatedBy = comment.AppUser?.UserName ?? "Nepoznata osoba jer nisi dohvatio nav atribut" // Zbog ovoga mora Include(c=> c.AppUser) u CommentRepository kada dohvatam comment iz baze jer je AppUser navigacioni atribut i nece biti automatic dohvacen isto vazi i za dohvatanje Stock iz baze tj Stock.Include(s => s.AppUser)
        ...
        public static Comment ToCommentFromCreateCommentRequestDTO(this CreateCommentCommandModel command, int stockId)
        {
            return new Comment
            {
                ...
                /* CreatedOn polje nisam mapirao, jer ne postoji u CreateCommentRequestDTO, pa bice DateTime.Now by default 
                   Nisam mapirao StockId jer u trenu mapiranja baza nije postavila Id polje za novi stock jer jos on nije ubacen.
