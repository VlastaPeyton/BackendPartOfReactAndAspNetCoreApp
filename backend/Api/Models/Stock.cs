...
{   // Models folder sluzi za Entity klase jer te klase ce biti tabele u bazi. 
    ...
    public class Stock // Entity jer predstavlja tabelu u bazi
    {
        ...
        public List<Comment> Comments { get; set; } = new();   // Navigation property => Include(Comments). Pogledaj IEnumerable vs List.txt
        public List<Portfolio> Portfolios { get; set; } = new List<Portfolio>(); // Navigation property => Include(Comments). Pogledaj IEnumerable vs List.txt
